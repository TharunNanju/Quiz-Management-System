...
export const assignQuizToStudent = async (quizId, studentId, dueDate) => {
    ...
    const [result] = await pool.execute(`INSERT INTO QuizAssignments (QuizID, StudentID, DueDate, Status)
     VALUES (?, ?, ?, 'assigned')
     ON DUPLICATE KEY UPDATE DueDate = VALUES(DueDate), Status = 'assigned'`.replace(/\s+/g, ' '), [quizId, studentId, dueDate ?? null]);
    const assignmentId = result.insertId || result.affectedRows;
    const [rows] = await pool.execute(`SELECT * FROM QuizAssignments WHERE QuizID = ? AND StudentID = ?`, [
        quizId,
        studentId
    ]);
